...
class Designnbuy_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    /**
	...
	/**
     * Create customer account action from designtool
     */
    public function createNewPostAction()
    {
		...
		$xmlString = array();
        if ($session->isLoggedIn()) {
			$xmlString['error'] = 'Already Login';
			$xmlString['status'] = 'false';
            echo json_encode($xmlString);
            exit;
        }
        $session->setEscapeMessages(true); // prevent XSS injection in user input
        if (!$this->getRequest()->isPost()) {
        ...
        try {
            ...
            if (empty($errors)) {
                ...
                $customer->save();
                $this->_dispatchRegisterSuccess($customer);
                $this->_successProcessRegistrationDesigntool($customer);
				$xmlString['id'] = $session->getCustomer()->getEntityId();
				$xmlString['status'] = 'true';
				echo json_encode($xmlString);
				exit;
                // return;
            } else {
				...
				foreach($errors as $error){
					$errorMsg[] = $this->__($error);
				}
				$errorString = implode(',',$errors);
				$xmlString['error'] = $errorString;
				$xmlString['status'] = 'false';
				echo json_encode($xmlString);
				exit;
                // $this->_addSessionError($errors);
			...
			$xmlString['error'] = $this->__('Cannot save the customer.');
			$xmlString['status'] = 'false';
			echo json_encode($xmlString);
			exit;
	...
	/**
     * Success Registration from designtool
     *
     * @param Mage_Customer_Model_Customer $customer
     * @return Mage_Customer_AccountController
     */
    protected function _successProcessRegistrationDesigntool(Mage_Customer_Model_Customer $customer)
    {
        $session = $this->_getSession();
        if ($customer->isConfirmationRequired()) {
            /** @var $app Mage_Core_Model_App */
	...
	/**
     * Customer login check from designtool
     */
	...
	public function loginFromToolAction()
    {
		...
		if (!empty($email) && !empty($password)) {
			...
			$xmlString['error'] = $this->__('Login and password are required.');
			$xmlString['status'] = 'false';
			echo json_encode($xmlString);
			exit;
		}
