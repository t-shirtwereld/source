<div class="pc_contten">
	...
	<div class="pc_contten box-box">
	    <form id="editCategoryForm" action="<?php echo BASE_ADMIN_URL . 'clipart_categories/submitdata'; ?>" method="post" class="col-two">
		<input type="hidden" name="clipart_category_id" value="<?php echo $category_row['clipart_category_id']; ?>" />
		<div class="input-area" style="margin-bottom:30px;">
			...
			<div class="field-form">
				...
				<input type="text" name="category_name[<?php echo $language['language_id']; ?>][name]" value="<?php echo isset($category_names[$language['language_id']]) ? htmlspecialchars($category_names[$language['language_id']]['name']) : ''; ?>" placeholder="" />
			...
			<!-- <div class="field-set">
				...
				<select name="parent_category_id">
					...
					if ($parentcategory['clipart_category_id'] != $category_row['clipart_category_id']) {
					    ?>
				    <?php if($parentcategory['status'] != '1') { ?> 
					    <option value="<?php echo $parentcategory['clipart_category_id']; ?>" <?php if ($parentcategory['clipart_category_id'] == $category_row['parent_category_id']) echo "selected=selected"; ?>><?php echo $parentcategory['name'].' ( Inactive )'; ?></option>
					<?php } else { ?>
					    <option value="<?php echo $parentcategory['clipart_category_id']; ?>" <?php if ($parentcategory['clipart_category_id'] == $category_row['parent_category_id']) echo "selected=selected"; ?>><?php echo $parentcategory['name']; ?></option>
					<?php } ?>
			...
			<div class="field-set">
				...
				<fieldset class="switch">
				    <label class="off">
					Inactive<input type="radio" class="on_off" name="status" value="0"/>
				    </label>
				    <label class="on">
					Active<input type="radio" class="on_off" name="status" value="1" />
				    </label>
				</fieldset>
			    </div>
			</div>
