<div class="pc_contten">
    <div class="pc_top">
        <h1>Fonts</h1>
	<div class="pc_rgt" style="margin-left: 10px;"><a href="<?php echo BASE_ADMIN_URL . 'fonts/addfont'; ?>">Add Font</a></div>
	<div class="pc_rgt"><a href="<?php echo BASE_ADMIN_URL . 'fonts/importcsv'; ?>">Import Fonts</a></div>
    </div>

    <div class="pc_search">
	<div class="switcher">
	    <form name="search" action="<?php echo BASE_ADMIN_URL . 'fonts/index/'; ?>" method="get">
		...
		<a href="<?php echo BASE_ADMIN_URL . 'fonts'; ?>" style="float: right; margin: 0px 8px; line-height: 36px;"><img src="<?php echo base_url('assets/images/recet.png'); ?>" alt=""></a>
	    </form>
	</div>
    </div>
	...
	<tbody>
		...
		foreach ($fonts as $c) {
		    ?>
		    <tr id="<?php echo $c['font_id']; ?>">
			<td><?php echo $c['font_id']; ?></td>
			<td><?php echo $c['font_name']; ?></td> 
			<td><?php echo $c['status'] == 1 ? 'Active' : 'Inactive'; ?></td>
			<td><a href="<?php echo BASE_ADMIN_URL . 'fonts/editfont/' . $c['font_id']; ?>">Edit</a>&nbsp;&nbsp;&nbsp;<a class="delete" href="<?php echo BASE_ADMIN_URL . 'fonts/deletefont/' . $c['font_id']; ?>">Delete</a></td>
		    </tr>
		    <?php
		}
	    } else {
...
<script>
	...
	$('#limit').bind('change', function () {
	    ...
	    var url = "<?php echo BASE_ADMIN_URL . 'fonts/index?keyword='.$keyword.'&limit='; ?>"+limit_val;
...
<?php if (empty($keyword) && empty($_GET['per_page'])) { ?>
    		...
    		$( "#sortable tbody" ).sortable({
    			...
    			$.ajax({
    			    type: "POST",
    			    async:true,
    			    url: "<?php echo BASE_ADMIN_URL . 'fonts/updateSortableRow'; ?>",
    			    data: stringDiv,
    			    error: function(){
