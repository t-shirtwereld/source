...
class Inkscape {
	...
	/* $tmpFileName =  sys_get_temp_dir().'/tmp.svg';          
	  ...
	  $filename = 'data:' . $ext . ';base64,' . $file;
	  if($ext=='png' || $ext=='jpeg' || $ext=='jpg'):
	  $tags->setAttribute('xlink:href',$filename);
	  endif;
	  endif;
	  endforeach;
	  endforeach;

	  $tempSvg = $doc->saveXML($doc);


	  file_put_contents($tmpFileName, $tempSvg);
	  $this->addParam( 'without-gui' );
	  $this->setFile( $tmpFileName ); */
    }
