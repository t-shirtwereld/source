 ...
 *                                        Added smallStep
 ...
 Sample usage:
	...
	$j(document).ready(function(){
		...
		$j("#myInputElement").SpinButton(myOptions);

	});
 
 */
$j.fn.SpinButton = function(cfg){
	return this.each(function(){

		this.repeating = false;
		
		// Apply specified options or defaults:
		// (Ought to refactor this some day to use $j.extend() instead)
		this.spinCfg = {
		...
		this.adjustValue = function(i){
			...
			} else if($j.isFunction(this.spinCfg.stepfunc)) {
			...
			this.value = v;
			if ($j.isFunction(this.spinCfg.callback)) this.spinCfg.callback(this);
		};
		...
		.mousemove(function(e){
			...
			if (direction !== this.spinCfg._direction) {
				...
				switch(direction){
					case 1: // Up arrow:
						$j(this).removeClass(this.spinCfg.downClass).addClass(this.spinCfg.upClass);
						break;
					case -1: // Down arrow:
						$j(this).removeClass(this.spinCfg.upClass).addClass(this.spinCfg.downClass);
						break;
					default: // Mouse is elsewhere in the textbox
						$j(this).removeClass(this.spinCfg.upClass).removeClass(this.spinCfg.downClass);
				}
	...
	function coord(el,prop) {
		...
		while ((el = el.offsetParent) && (el != b)) {
			if (!$j.browser.msie || (el.currentStyle.position != 'relative'))
				c += el[prop];
		}
