...
var imagewidth = 400;
var imageheight = 485;
var jasonStr = JSON.parse(jasonStrthreed);
var tabId="#tab-5";
var saveCoordinate = saveCoordinate3d;
...
function loadSideData(){
	...
	$(tabId).append(divCon);
	$.each(jasonStr.coordinatethreed,function(index,value){
		var sideId = "side_threed"+(index+1);
...
function createSideButtons(){
	...
	// console.log(jasonStr.coordinatethreed);
	$.each(jasonStr.coordinatethreed,function(index,value){
...
function createInputsBox(){
	...
	label = 'Output Width';
    inputBoxOW = $('<input type="hidden"  type="text" />');
	...
	label = 'Output Height';
	inputBoxOH = $('<input type="hidden" type="text" />');
...
function hidAllSides(){
	$.each(jasonStr.coordinatethreed,function(index,value){
		var imageId ='side_threed'+(index+1);
		$('#'+imageId).hide();
	});
}
function showSide(evt){
	...
	$("#side_threed"+imageId).show();
	currentSide = ("side_threed"+imageId);
	//setParameters();
...
function saveSideData(evt){
	...
	$.ajax({
		...
		success: function (response) {
		    if(response === 'true') {
			$('#savebtnCon3d').after("<p id='are_suc'>Config Area Saved Successfully...!</p>");
			$('#are_suc').fadeOut(2000);
		    } else {
			$('#savebtnCon3d').after("<p id='are_err'>Error Saving Config Area...!</p>");
